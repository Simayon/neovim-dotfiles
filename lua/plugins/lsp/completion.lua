							...
							label = {
								text = function(ctx)
									...
									return ctx.label
										.. " <"
										.. string.gsub(
											string.sub(ctx.label_detail, code_start, code_end),
											"  ·  ",
											" "
										)
										.. ">"
			...
			keymap = {
				...
				["z"] = {
					function(cmp)
						...
						if content_before_cursor:match("z%w*$") then
							return false
						end
					...
					lsp = {
						...
						transform_items = function(context, items)
							...
							return vim.tbl_filter(function(item)
								...
								if not rime_ls.is_rime_item(item) and item.kind ~= TYPE_ALIAS.Text then
									return true
								end
