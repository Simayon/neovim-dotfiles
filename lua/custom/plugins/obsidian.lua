return {
  ...
  opts = {
    ...
    callbacks = {
      ...
      --     local path = tostring(workspace.path)
      ...
      --     end
      --   else
      --     print("Error: Invalid workspace or workspace path")
      --   end
      -- end,
    },
    ...
    vim.api.nvim_create_user_command('ObsidianSwitchWorkspace', function()
      ...
      vim.ui.select(opts.workspaces, {
        ...
        if workspace then
          ...
          vim.cmd('ObsidianWorkspace ' .. workspace.name)

          -- Change the working directory
          local path = vim.fn.expand(workspace.path)
          if vim.fn.isdirectory(path) == 1 then
            vim.api.nvim_set_current_dir(path)
            print("Switched to workspace '" .. workspace.name .. "' and changed working directory to: " .. path)
          else
            print('Error: Directory does not exist: ' .. path)
          end
        end
